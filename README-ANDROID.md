...
### v5.28.2 — FIX: vinculación duradera de la cuenta de Google + reintentos de subida

- **FIX (vinculación perdida al reabrir):** la cuenta vivía SOLO en el localStorage del WebView. Si el sistema detenía la app en frío (HyperOS/MIUI), se actualizaba la APK o se limpiaba el WebView, ese almacenamiento se perdía y la "vinculación" desaparecía. Ahora `GoogleDrivePlugin` expone `guardarCuenta/leerCuenta/borrarCuenta` sobre **SharedPreferences nativas** (sobreviven a todo salvo desinstalar), `drive.js` guarda en ambos lados y `init()` restaura la cuenta desde el lado nativo si el localStorage la perdió.
- **FIX (subidas silenciosamente perdidas):** el debounce de 15 s moría si cerrabas la app antes de dispararse, y los fallos con `silencioso=true` no avisaban. Ahora hay **reintento automático cada 5 min** mientras la app viva (el servicio en primer plano la mantiene), reintento al volver la red, y el estado en Ajustes muestra "⏳ copia pendiente" o el error concreto "⚠️ …". Si la 1.ª copia tras conectar falla, se avisa con toast.
- **Nota de verificación:** la copia vive en "Datos de aplicaciones ocultos" (appDataFolder) — NO aparece navegando Drive normalmente; la forma de verificarla es la línea de estado en Ajustes (fecha de última copia o error).
